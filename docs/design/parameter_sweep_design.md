...
The parameter-sweep runner evaluates a finite YAML Cartesian grid through the normal registry-driven experiment pipeline. Every parameter-set, instance, seed, and repeat creates an immutable source run below the active level's output directory. Algorithm settings use `sweep.parameters`; level/model settings use `sweep.config_parameters` with dotted paths such as `support.threshold`. Overrides are written into that run's `resolved_config.yaml`; the aggregate never uses hidden notebook state or temporary source configs.
...
Unknown algorithm parameters, unknown or non-scalar config paths, duplicate grid values, empty dimensions, duplicate/negative seeds, and grids above `max_parameter_sets` are rejected before an aggregate run directory is created. Config sweeps may only override `model`, `support`, `solver`, or `validation`; they cannot change paths, data selection, output roots, or container definitions.
